...
public class GridVLineModel extends GridLineModel {
    ...
    private VLineModel createVLine(PanelModel panel) {
        ...
        if (panel.getRatioMinX().equals(ratioXProperty.get())) {
            ...
            panel.ratioMaxXObservable.filter(ratioMaxX -> !ratioMaxX.equals(ratioXProperty.get())).takeUntil(vLineRemovedObservable.filter(vLine::equals)).subscribe(ratioMaxX -> vLines.remove(vLine));
        }
